            ...
            try {
                ...
                if (localId != -1L) {
                    localDataSource.updateDoc(
                        localDataSource.getDoc(localId).copy(status = DocStatus.SENDING)
                    )
                    //upload to server
                    ...
                    localDataSource.updateDoc(
                        localDataSource.getDoc(localId).copy(status = DocStatus.SENT)
                    )
                    return@withContext Result.success()
                } else {
                    Timber.d("documento não encontrado")
                    localDataSource.updateDoc(
                        localDataSource.getDoc(localId).copy(status = DocStatus.NOT_SENT)
                    )
